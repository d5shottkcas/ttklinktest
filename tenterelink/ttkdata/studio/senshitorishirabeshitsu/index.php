...
<head>
  ...
  <title>戦士取調室一覧表 - 天てれリンクプラス</title>
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="col-lg-12">
        ...
        <header>
          <h1 class="pagetitle jp" lang="ja-jp">戦士取調室一覧表</h1>
          <h1 class="pagetitle en" lang="en-gb">戦士取調室一覧表</h1>
          <h1 class="pagetitle tw" lang="zh-tw">戦士取調室一覧表</h1>
          <h1 class="pagetitle tl" lang="tl-ph">戦士取調室一覧表</h1>
        </header>
    ...
    <div class="row">
      ...
      <div class="col-lg-9 order-lg-2">
        ...
        <h3 class="h2-border text-left">戦士取調室</h3>

                <p class="text-left small">※表は左右にスクロールできます。</p>

        <table class="table table-bordered-cg table-responsive text-nowrap mx-auto bace text-555555 wh-bd-none small" summary="戦士取調室一覧表">

          <tr>
            <th colspan="4" class="th-white text-left">戦士取調室 2009　刑事：半田健人</th>
          </tr>
          <tr>
            <th class="th-i text-center"> </th>
            <th class="th-i text-center">てれび戦士</th>
            <th class="th-i text-center">罪</th>
            <th class="th-i text-center">放送日</th>
          </tr>

          <?php
                $lines = file('csv/torishirabe2009-01.csv');
                $count = count($lines);
            for($i=0;$i<$count;$i++){
                  ...
                  echo '<td>'.$csv[1].'</td>';
                  echo '<td>'.$csv[2].'</td>';
                  echo '<td class="text-left px-1">'.$csv[3].'</td>';
                  echo '</tr>';
                }
                ?>

          <tr>
            <th colspan="4" class="th-white text-left">戦士取調室 2009　刑事：小原正子</th>
          </tr>

          <?php
                $lines = file('csv/torishirabe2009-02.csv');
                $count = count($lines);
            for($i=0;$i<$count;$i++){
          ...
          <tr>
            <th colspan="4" class="th-white text-left">戦士取調室 2009　刑事：スミッコ</th>
          </tr>

          <?php
                $lines = file('csv/torishirabe2009-03.csv');
                $count = count($lines);
            for($i=0;$i<$count;$i++){
          ...
          <tr>
            <th colspan="4" class="th-white text-left">戦士取調室 2010　刑事：柳沢慎吾</th>
          </tr>

          <?php
                $lines = file('csv/torishirabe2010.csv');
                $count = count($lines);
            for($i=0;$i<$count;$i++){
