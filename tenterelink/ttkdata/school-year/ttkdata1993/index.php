<!DOCTYPE html>
<html lang="ja">

<head>

  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <meta name="description" content="天才てれびくん関連サイトリンク集。てれび戦士や天てれの公式サイトやブログ、ファンサイトや応援ブログなどを収集。てれび戦士のプロフィールやMTK一覧、天てれの情報、掲示板などのコンテンツも豊富。">
  <meta name="keywords" content="天才てれびくんhello,天才てれびくんYOU,Let's天才てれびくん,大!天才てれびくん,天才てれびくんMAX,天てれ,てれび戦士,Dream5,">

  <title>1993年度 - 天てれリンクプラス</title>

  <meta name="title" content="天てれリンクプラス - 天才てれびくんhello,関連リンク集">
  <meta property="og:type" content="website" />
  <meta property="og:title" content="天てれリンクプラス - 天才てれびくんhello,関連リンク集" />
  <meta property="og:description" content="天才てれびくん関連サイトリンク集。てれび戦士や天てれの公式サイトやブログ、ファンサイトや応援ブログなどを収集。てれび戦士のプロフィールやMTK一覧、天てれの情報、掲示板などのコンテンツも豊富。">
  <meta property="og:url" content="https://www.tenterelink.net/" />
  <meta property="og:site_name" content="天てれリンクプラス" />
  <meta property="og:image" content="https://www.tenterelink.net/sozai/tenterelink-logo.png" />
  <meta name="twitter:card" content="summary" />
  <meta name="twitter:site" content="@tenterelink" />

  <link href="../../../css/bootstrap.css" rel="stylesheet">
  <link href="../../../css/tenterelink.css" rel="stylesheet">
  <link href="../../../css/lang-jp.css" rel="stylesheet" id="switchCSS">

  <link rel="shortcut icon" href="https://www.tenterelink.net/sozai/favicon.ico" />
  <!--[if lt IE 9]>
<script src="http://html5shiv.googlecode.com/svn/trunk/html5.js"></script>
<script src="http://css3-mediaqueries-js.googlecode.com/svn/trunk/css3-mediaqueries.js"></script>
<![endif]-->

</head>

<body onload="scrollTest()">

  <nav class="navbar navbar-expand-lg navbar-navi fixed-top">
    <div class="container">
      <a class="btn btn-title" href="../../../">天てれリンク</a>
      <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarResponsive" aria-controls="navbarResponsive"
        aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
      </button>
      <div class="collapse navbar-collapse" id="navbarResponsive">
        <ul class="navbar-nav ml-auto">
          <?php
            $lines = file('../../../csv/menu04.csv');
            $count = count($lines);
            for($i=0;$i<$count;$i++){

              $csv = explode(',',$lines[$i]);
              echo '<li class="nav-item">'.$csv[0].$csv[1].$csv[2].$csv[3].$csv[4].'</a>'.'</li>';
            }
            ?>
        </ul>
      </div>
    </div>
  </nav>

  <div class="w-100 py-1">
    <div class="container">
      <div class="clearfix">
        <div class="float-right">
          Language：
          <select id="selectCSS">
            <option value="lang-jp">日本語</option>
            <option value="lang-en">English</option>
            <option value="lang-tw">漢語</option>
            <option value="lang-tl">Tagalog</option>
          </select>
        </div>
      </div>
    </div>
  </div>

  <div class="container">

    <div class="row">

      <div class="col-lg-12">

        <header>
          <h1 class="pagetitle jp" lang="ja-jp">1993年度</h1>
          <h1 class="pagetitle en" lang="en-gb">1993</h1>
          <h1 class="pagetitle tw" lang="zh-tw">1993年度</h1>
          <h1 class="pagetitle tl" lang="tl-ph">1993</h1>
        </header>

      </div>


    </div>
    <!-- /.row -->

    <div class="row">

      <div class="col-lg-12">

      <div class="my-2">
        <script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
        <!-- 天てれリンク自動 -->
        <ins class="adsbygoogle ins_ad" style="display:block" data-ad-client="ca-pub-9359745436138089" data-ad-slot="8890141261"
          data-ad-format="auto"></ins>
        <script>
          (adsbygoogle = window.adsbygoogle || []).push({});
        </script>
        </div>

      </div>

    </div>
    <!-- /.row -->

    <div class="row">

      <div class="col-lg-9 order-lg-2">

        <table class="ttkdata">
          <tr>
            <th rowspan="3">1993年度データ</th>
            <td colspan="2">時は２１世紀…<br>人間とリモコンの言いなりになってきたテレビがついに反乱を起こした<br> ブラウン管がカンカンに怒り カラーバー光線が人々を襲う<br>そんなメディアの戦場に今 ９人のてれび戦士が立ち向かおうとしていた
            </td>
          </tr>
          <tr>
            <td colspan="2">月-木曜日　１８時～１８時２５分</td>
          </tr>
          <tr>
            <td colspan="2">初回放送　　１９９３／０４／０５</td>
          </tr>

          <tr>
            <td class="ttkdata">司会者</td>
            <td colspan="2">ダチョウ倶楽部（おあいこトリオ）</td>
          </tr>

          <tr>
            <td class="ttkdata">てれび戦士</td>
            <td colspan="2">
              <ul>
                <li>クリスティー・コーサル</li>
                <li>須山彩</li>
                <li>清野努</li>
                <li>栗山祐哉</li>
                <li>小林一裕</li>
                <li>山口美沙</li>
                <li>ジェニファー・ハースト</li>
                <li>中條慎太郎</li>
                <li>田原加奈子</li>
              </ul>
          </tr>

          <tr>
            <td class="ttkdata">レギュラー出演者</td>
            <td colspan="2">石川磨衣、鈴木加奈枝、中村友里子、小畠郁生</td>
          </tr>
          <tr>
            <td class="ttkdata">ナレーション</td>
            <td colspan="2">中村秀利</td>
          </tr>
          <tr>
            <td class="ttkdata">主なゲスト</td>
            <td colspan="2"></td>
          </tr>
          <tr>
            <td class="ttkdata">スタジオCGキャラ</td>
            <td colspan="2">てっちゃん</td>
          </tr>
          <tr>
            <td class="ttkdata">メインCGキャラ 声担当</td>
            <td colspan="2">千葉繁</td>
          </tr>
          <tr>
            <td class="ttkdata">その他CGキャラ</td>
            <td colspan="2">アッケラ缶、ポコ</td>
          </tr>
          <tr>
            <td class="ttkdata">舞台設定</td>
            <td colspan="2">メディアタワー</td>
          </tr>

          <tr>
            <td rowspan="3" class="ttkdata w20p">主なコーナー</td>
            <td class="ttkdatay w20p">恐竜惑星</td>
            <td class="w60p">バーチャルアニメ</td>
          </tr>
          <tr>
            <td class="ttkdatay">ポコ・ア・ポコ</td>
            <td>クリス担当の音楽コーナー。</td>
          </tr>
          <tr>
            <td class="ttkdatay">この人に会いたい</td>
            <td>いろんな芸能人を訪問する。</td>
          </tr>

          <tr>
            <td rowspan="3" class="ttkdata">音楽関係</td>
            <td class="ttkdatay">オープニング</td>
            <td>タイムマシーンでいこう</td>
          </tr>
          <tr>
            <td class="ttkdatay">エンディング</td>
            <td>YOU YOU YOU</td>
          </tr>
          <tr>
            <td class="ttkdatay">ポコ・ア・ポコ</td>
            <td>
              <ul>
                <li>先生は胴長おじさん</li>
                <li>ジャンピング・ジャック・フラッシュ</li>
                <li>運命</li>
                <li>世界中の誰よりきっと</li>
                <li>ダイヤモンド</li>
                <li>年下の男の子</li>
              </ul>
          </tr>

          <tr>
            <td rowspan="4" class="ttkdata">イベント</td>
            <td class="ttkdatay">夏</td>
            <td></td>
          </tr>
          <tr>
            <td class="ttkdatay">冬</td>
            <td>ポコ・ア・ポコ コンサート</td>
          </tr>
          <tr>
            <td class="ttkdatay">教育フェア</td>
            <td></td>
          </tr>
          <tr>
            <td class="ttkdatay">その他</td>
            <td>紅白歌合戦</td>
          </tr>

          <tr>
            <td class="ttkdata">ホームページ企画</td>
            <td colspan="2">－</td>
          </tr>
          <tr>
            <td class="ttkdata">天てれグッズ</td>
            <td class="ttkdatay">CD</td>
            <td>タイムマシーンでいこう／YOU YOU YOU</td>
          </tr>
          <tr>
            <td class="ttkdata">備考</td>
            <td colspan="2">おあいこトリオを呼び出す合言葉「ムッシュムラムラストッピング～！」<br>衣装が前半後半の２着あった。<br>おあいこトリオは最初は頭だけだった。<br>「大晦日だよ！天才てれびくん」「お正月だよ！天才てれびくん」放送</td>
          </tr>
        </table>

        <p>&nbsp;</p>
      </div>

      <div class="col-lg-3 order-lg-1">

        <div class="smenu">
          <ul class="smenu">
            <?php
              $lines = file('../csv/ttkdata-yearlist02.csv');
              $count = count($lines);
            for($i=0;$i<$count;$i++){
  
                $csv = explode(',',$lines[$i]);
                echo '<li class="smenu">'.$csv[0].$csv[1].'</a>'.'</li>';
              }
              ?>
          </ul>
        </div>

      </div>
    </div>
    <!-- /.row -->

    <div class="row">

      <div class="col-lg-12">

      <div class="my-2">
        <script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
        <!-- 天てれリンク自動 -->
        <ins class="adsbygoogle ins_ad" style="display:block" data-ad-client="ca-pub-9359745436138089" data-ad-slot="8890141261"
          data-ad-format="auto"></ins>
        <script>
          (adsbygoogle = window.adsbygoogle || []).push({});
        </script>
        </div>

      </div>

    </div>
    <!-- /.row -->

  </div>
  <!-- /.container -->


  <footer class="py-5 bg-orange">
    <div class="container">
      <p class="m-0 text-center text-dark">since 6.26.2001 by ichiro (since 3.25.2005 by Giabbit)
        <br> Copyright &copy; 2005 天てれリンク. All Rights Reserved. </p>
    </div>
    <!-- /.container -->
  </footer>

  <!-- Bootstrap core JavaScript -->
  <script src="../../../js/jquery.js"></script>
  <script src="../../../js/bootstrap.js"></script>
  <script src="../../../js/js.cookie.js"></script>
  <script src="../../../js/lang-hierarchy03.js"></script>

</body>

</html>