...
<head>
  ...
  <title>ウキウキ木曜！ナマ日記2010 4月8日 - 天てれリンクB号館</title>
...
<body>
  ...
  <div class="container">
    ...
    <!-- 投稿日 -->
    <h1 class="mt-4 h2">ウキウキ木曜！ナマ日記2010 4月8日</h1>
    <hr>
...
■２０１０年　４月８日（木）
          ...
          <h5 class="card-header">
            <?php
            $lines = file('csv/namanikki20100408-01.csv');
            $count = count($lines);
            $i=0;
              $csv = explode(',',$lines[$i]);
              echo $csv[0];
          ...
          <h5 class="card-header">
            <?php
            $lines = file('csv/namanikki20100408-02.csv');
            $count = count($lines);
            $i=0;
              $csv = explode(',',$lines[$i]);
              echo $csv[0];
          ...
          <h5 class="card-header">
            <?php
            $lines = file('csv/namanikki20100408-03.csv');
            $count = count($lines);
            $i=0;
              $csv = explode(',',$lines[$i]);
              echo $csv[0];
          ...
          <h5 class="card-header">
            <?php
            $lines = file('csv/namanikki20100408-04.csv');
            $count = count($lines);
            $i=0;
              $csv = explode(',',$lines[$i]);
              echo $csv[0];
